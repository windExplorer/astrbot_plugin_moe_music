...
命令 handler（main.py）只做参数解析与转发；本模块负责与后端交互、
候选列表展示、session_waiter 选歌等待、歌词渲染与异常兜底。
...
class MoeMusicService:
    ...
    async def handle_song_request(
        ...
        """处理一次点歌请求（搜索 → 直发 / 候选列表 → 等待选号 → 发送）。"""
        try:
            tracks = await self.api.search(
                keyword=keyword,
                limit=self.cfg.song_limit,
                source=source or None,
                quality=self.cfg.default_quality,
            )
        ...
        if 0 < index_hint <= len(tracks):
            await self.sender.send_track(event, tracks[index_hint - 1])
            return
    ...
    async def _wait_for_selection(self, event: AstrMessageEvent, tracks: list[Track]) -> None:
        ...
        async def song_picker(controller: SessionController, ev: AstrMessageEvent):
            ...
            controller.stop()
            ev.stop_event()
            await self.sender.send_track(ev, tracks[n - 1])
    ...
    async def handle_lyrics_request(self, event: AstrMessageEvent, keyword: str) -> bool:
        """查歌词：搜索第一首 → 取歌词 → 渲染图片发送。

        Returns:
            bool: 是否成功发送。
        """
        track = await self._search_first(event, keyword)
        if track is None:
            return False
    ...
    async def _search_first(self, event: AstrMessageEvent, keyword: str) -> Track | None:
        """搜索并返回第一首，无结果时向用户发送提示。"""
        try:
            tracks = await self.api.search(
                keyword=keyword,
                limit=1,
    ...
    async def llm_play_song(self, event: AstrMessageEvent, song_name: str, source: str = "") -> str:
        ...
        try:
            ...
            logger.error(f"[萌音点歌] AI 点歌搜索失败：code={e.code}（关键词：{song_name}）")
            return f"点歌失败：{e.user_hint}"
        if not tracks:
            return f"没有找到《{song_name}》相关的歌曲"
        track = tracks[0]
        sent = await self.sender.send_track(event, track)
        if not sent:
            return "歌曲发送失败，请稍后再试"
    ...
    async def llm_query_lyrics(self, event: AstrMessageEvent, song_name: str) -> str:
        ...
        track = await self._search_first(event, song_name)
        if track is None:
            return f"没有找到《{song_name}》相关的歌曲"
        sent = await self.send_lyrics_for_track(event, track)
        if not sent:
            return "歌词获取或发送失败"
        return f"已为用户展示《{track.name}》- {track.singer} 的歌词"
